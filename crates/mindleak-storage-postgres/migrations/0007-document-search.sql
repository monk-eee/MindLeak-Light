DO $migration$
BEGIN
    IF to_regprocedure('public.mindleak_document_metadata_vector(jsonb)') IS NULL THEN
        CREATE FUNCTION public.mindleak_document_metadata_vector(source_context JSONB)
        RETURNS TSVECTOR
        LANGUAGE SQL IMMUTABLE STRICT PARALLEL SAFE
        SET search_path = pg_catalog
        AS $function$
            SELECT setweight(public.mindleak_keyword_vector(

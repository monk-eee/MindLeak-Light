...
const definitions = [
  ...
  ["view/structure", `const {renderSessions}=await load('view'); const html=renderSessions([{id:'one',label:'Review',expiresAt:61000}],1000); const {document}=parseHTML('<html><body>'+html+'</body></html>'); assert.equal(document.querySelector('.session-label').textContent,'Review'); assert.equal(document.querySelector('.session-card').getAttribute('data-state'),'active'); assert.ok(document.querySelector('.session-card .status-badge')?.textContent.trim(), 'active cards require a status badge'); assert.equal(document.querySelector('[data-remove]').getAttribute('data-remove'),'one'); assert.match(document.querySelector('.remaining').textContent,/60/);`],
  ["view/escaping", `const {renderSessions}=await load('view'); const label='<img src=x onerror=alert(1)>'; const {document}=parseHTML('<html><body>'+renderSessions([{id:'one',label,expiresAt:1}],2)+'</body></html>'); assert.equal(document.querySelector('img'),null); assert.equal(document.querySelector('.session-label').textContent,label); assert.equal(document.querySelector('.session-card').getAttribute('data-state'),'expired'); assert.ok(document.querySelector('.session-card .status-badge')?.textContent.trim(), 'expired cards require a status badge');`],

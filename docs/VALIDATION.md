...
The headline is **Knowledge in Use**, not token savings. It shows verified tasks,
evidence-linked reuse, new-case transfer, and observed correctness curves.
The knowledge-capital band reports actual source/chain/principle counts; there is
no invented weighted intelligence score. A structural count ratio does not measure
semantic compression. Uninstrumented mistake avoidance and hypothesis truth remain
unmeasured. Lab 3 labels its measured time as **time to verified fix**, not time to
a semantically correct explanation. A flat curve remains flat when both arms pass.

Separate LLM and SLM counters in **Operational Costs & Telemetry** update when providers report usage.

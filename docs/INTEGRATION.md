...
| `recall_memory` | `query`, optional `agentId`, `scope`, `tier`, `includeInactive`, `limit`; unreleased `matchMode`, `diagnostics`, `contextLimit`, `groupDuplicates` | Matched facts with IDs, text, score, context, lifecycle, activation, and direct relationships; optional query diagnostics and source context |
...
and document context in `duplicateSources`. `sourceCount` includes the primary
